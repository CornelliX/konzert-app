(function(){const t=document.createElement("link").relList;if(t&&t.supports&&t.supports("modulepreload"))return;for(const r of document.querySelectorAll('link[rel="modulepreload"]'))s(r);new MutationObserver(r=>{for(const d of r)if(d.type==="childList")for(const c of d.addedNodes)c.tagName==="LINK"&&c.rel==="modulepreload"&&s(c)}).observe(document,{childList:!0,subtree:!0});function n(r){const d={};return r.integrity&&(d.integrity=r.integrity),r.referrerPolicy&&(d.referrerPolicy=r.referrerPolicy),r.crossOrigin==="use-credentials"?d.credentials="include":r.crossOrigin==="anonymous"?d.credentials="omit":d.credentials="same-origin",d}function s(r){if(r.ep)return;r.ep=!0;const d=n(r);fetch(r.href,d)}})();const M="https://cqxpjesovcavarrlljpn.supabase.co",S="eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6ImNxeHBqZXNvdmNhdmFycmxsanBuIiwicm9sZSI6ImFub24iLCJpYXQiOjE3NzI0NjI4MjYsImV4cCI6MjA4ODAzODgyNn0._paXbtV1uecR0AA08KUkKLfRkmkQw2k4W3hT5L2y2ho";async function _(){const e=await fetch(`${M}/rest/v1/manual_events?select=*&order=date.asc`,{headers:{apikey:S,Authorization:"Bearer "+S}});return e.ok?(await e.json()).map(n=>({id:n.id,title:n.title,type:n.type,date:n.date,time:n.time,locationId:n.location_id,locationName:n.location_name,locationCity:n.location_city,description:n.description,ticketUrl:n.ticket_url,spotifyUrl:n.spotify_url,source:"manual"})):[]}async function U(e,t,n){return(await fetch(`${M}/rest/v1/manual_events`,{method:"POST",headers:{apikey:S,Authorization:"Bearer "+S,"Content-Type":"application/json",Prefer:"return=minimal"},body:JSON.stringify({id:e.id,title:e.title,type:e.type,date:e.date,time:e.time,location_id:e.locationId,location_name:t,location_city:n,description:e.description,ticket_url:e.ticketUrl,spotify_url:e.spotifyUrl})})).ok}async function q(){const e=new Date().toISOString().split("T")[0];await fetch(`${M}/rest/v1/manual_events?date=lt.${e}`,{method:"DELETE",headers:{apikey:S,Authorization:"Bearer "+S}})}function z(e){const t=localStorage.getItem(e);return t?JSON.parse(t):null}function L(e,t){localStorage.setItem(e,JSON.stringify(t))}function P(){const e=z("locations");if(e)return e;const t=[{id:1,name:"Lido",city:"Berlin",website:"lido-berlin.de",capacity:400},{id:2,name:"SO36",city:"Berlin",website:"so36.com",capacity:600},{id:3,name:"Festsaal Kreuzberg",city:"Berlin",website:"festsaal-kreuzberg.de",capacity:1200},{id:4,name:"Privatclub",city:"Berlin",website:"privatclub-berlin.de",capacity:250},{id:5,name:"Astra Kulturhaus",city:"Berlin",website:"astra-berlin.de",capacity:1800},{id:6,name:"Frannz Club",city:"Berlin",website:"frannz.eu",capacity:350},{id:7,name:"Monarch",city:"Berlin",website:"kottimonarch.de",capacity:150},{id:8,name:"Musik & Frieden",city:"Berlin",website:"musikundfrieden.de",capacity:700},{id:9,name:"Wild at Heart",city:"Berlin",website:"wildatheartberlin.de",capacity:280},{id:10,name:"Columbia Theater",city:"Berlin",website:"columbia-theater.de",capacity:800},{id:11,name:"Schokoladen",city:"Berlin",website:"schokoladen-mitte.de",capacity:150},{id:12,name:"Madame Claude",city:"Berlin",website:"madameclaude.de",capacity:100},{id:13,name:"Conne Island",city:"Leipzig",website:"conne-island.de",capacity:600},{id:14,name:"Werk 2",city:"Leipzig",website:"werk-2.de",capacity:500},{id:15,name:"Täubchenthal",city:"Leipzig",website:"taeubchenthal.com",capacity:1200},{id:16,name:"Felsenkeller",city:"Leipzig",website:"felsenkeller-leipzig.com",capacity:2e3},{id:17,name:"UT Connewitz",city:"Leipzig",website:"utconnewitz.de",capacity:350},{id:18,name:"Moritzbastei",city:"Leipzig",website:"moritzbastei.de",capacity:500},{id:19,name:"Horns Erben",city:"Leipzig",website:"horns-erben.de",capacity:150},{id:20,name:"Ilses Erika",city:"Leipzig",website:"",capacity:200},{id:21,name:"Urban Spree",city:"Berlin",website:"urbanspree.com",capacity:250},{id:22,name:"Gretchen",city:"Berlin",website:"gretchen-club.de",capacity:500},{id:23,name:"Supamolly",city:"Berlin",website:"supamolly.de",capacity:150},{id:24,name:"Kantine am Berghain",city:"Berlin",website:"berghain.berlin",capacity:200},{id:25,name:"Tempodrom",city:"Berlin",website:"tempodrom.de",capacity:4200},{id:26,name:"Heimathafen Neukölln",city:"Berlin",website:"heimathafen-neukoelln.de",capacity:450},{id:27,name:"Bi Nuu",city:"Berlin",website:"binuu.de",capacity:400},{id:28,name:"Mikropol",city:"Berlin",website:"mikropol-berlin.de",capacity:300}];return L("locations",t),t}async function B(){try{const e=await fetch("./events.json"),t=e.ok?await e.json():[],n=await _();return[...t,...n]}catch(e){return console.log("Fehler beim Laden:",e),[]}}let m=P(),f=[],j=null,l={cities:["Berlin","Leipzig"],type:"alle",locationId:"alle",dates:[]},x=z("bookmarked")||[],k=z("going")||[],D=z("seenEvents")||[],E="liste",A=0;async function F(e){j=e,f=await B(),b()}function C(e){return!D.includes(e.id)}function R(){D=f.map(e=>e.id),L("seenEvents",D)}function b(){const e=f.filter(t=>C(t)).length;j.innerHTML=`
  <div id="ptr-indicator" style="text-align:center; height:0; overflow:hidden; transition:height 0.2s; color:rgba(255,255,255,0.5); font-size:13px; display:flex; align-items:center; justify-content:center;">↻ Aktualisieren...</div>
    ...
    ${H()}
  `,Z(),e>0&&E==="liste"&&setTimeout(()=>R(),3e3)}function K(e){return`
  ...
  `}function N(){const e=m.filter(t=>l.cities.includes(t.city)).sort((t,n)=>t.name.localeCompare(n.name));return`
    <div class="glass rounded-2xl p-4 mb-5 space-y-3">
      ...
      <div style="position:relative;">
        <div id="filter-loc-selected" style="cursor:pointer; padding:10px 14px; border-radius:12px; background:rgba(255,255,255,0.05); border:1px solid rgba(255,255,255,0.08); display:flex; justify-content:space-between; align-items:center;">
          <span style="font-size:14px; color:rgba(255,255,255,0.5);">${l.locationId==="alle"?"Alle Locations":m.find(t=>t.id==l.locationId)?.name||"Alle Locations"}</span>
    ...
    <div class="space-y-8">
      ${Object.entries(n).map(([s,r])=>`
        <div>
          ...
          <div class="space-y-3">${r.map(d=>T(d)).join("")}</div>
        </div>
      `).join("")}
    </div>
  `}function T(e){const t=m.find(i=>i.id===e.locationId),n=x.includes(e.id),s=k.includes(e.id),r=C(e),c=new Date(e.date+"T12:00:00").toLocaleDateString("de-DE",{weekday:"short",day:"numeric",month:"short"}),g=e.type==="konzert",u=g?"#818cf8":"#fb923c";return`
  ...
  `}function J(){const e=new Date,t=new Date(e.getFullYear(),e.getMonth()+A,1),n=t.getFullYear(),s=t.getMonth(),r=new Date(n,s+1,0).getDate(),d=(new Date(n,s,1).getDay()+6)%7,c=t.toLocaleDateString("de-DE",{month:"long",year:"numeric"}),g=O(),u=g.map(i=>i.date),a=g.filter(i=>l.dates.length===0||l.dates.includes(i.date));return`
    ${N()}
    <div class="glass rounded-2xl p-5 mb-5">
      ...
      <div class="grid grid-cols-7 gap-1">
        ...
        ${Array.from({length:r},(i,o)=>{const p=o+1,y=`${n}-${String(s+1).padStart(2,"0")}-${String(p).padStart(2,"0")}`,$=u.includes(y),w=l.dates.includes(y),h=p===e.getDate()&&s===e.getMonth()&&n===e.getFullYear(),I=new Date(y)<e&&!h;return`
            <button data-date="${y}" style="aspect-ratio:1; border-radius:12px; font-size:14px; font-weight:500; display:flex; align-items:center; justify-content:center; position:relative; transition:all 0.15s; ${I?"opacity:0.25;":""} color:${!$&&!w&&!h?"rgba(255,255,255,0.2)":"white"}; background:${w?"rgba(99,102,241,0.5)":$?"rgba(99,102,241,0.12)":"transparent"}; border:${w?"1px solid rgba(99,102,241,0.6)":$?"1px solid rgba(99,102,241,0.25)":h?"1px solid rgba(255,255,255,0.2)":"1px solid transparent"}; padding:4px; cursor:pointer;">
              ${p}
              ${$&&!w?'<span style="position:absolute;bottom:3px;left:50%;transform:translateX(-50%);width:3px;height:3px;border-radius:50%;background:#818cf8;"></span>':""}
            </button>
          `}).join("")}
      </div>
      ${l.dates.length>0?`
    ...
    <div class="space-y-3">
      ${l.dates.length>0&&a.length===0?'<p class="text-center text-slate-600 py-6 text-sm">Keine Events an den gewählten Tagen.</p>':a.map(i=>T(i)).join("")}
    </div>
  `}function Y(){const e=f.filter(s=>x.includes(s.id)),t=f.filter(s=>k.includes(s.id)),n=(s,r,d)=>`
    <div>
      <div class="flex items-center gap-3 mb-4">
        <div class="h-px flex-1" style="background:rgba(255,255,255,0.06);"></div>
        <span class="syne text-xs tracking-widest uppercase" style="color:rgba(255,255,255,0.25); font-weight:700;">${s}</span>
      ...
      ${r.length===0?`<p class="text-slate-700 text-sm py-4 text-center">${d}</p>`:`<div class="space-y-3">${r.map(c=>T(c)).join("")}</div>`}
    </div>
  `;return`
  ...
  `}const v="width:100%; border-radius:12px; padding:10px 14px; font-size:14px; color:white; outline:none; background:rgba(255,255,255,0.06); border:1px solid rgba(255,255,255,0.1); font-family:'DM Sans',sans-serif; color-scheme:dark;";function H(){const e=m.slice().sort((t,n)=>t.name.localeCompare(n.name));return`
