(function(){const t=document.createElement("link").relList;if(t&&t.supports&&t.supports("modulepreload"))return;for(const s of document.querySelectorAll('link[rel="modulepreload"]'))l(s);new MutationObserver(s=>{for(const c of s)if(c.type==="childList")for(const p of c.addedNodes)p.tagName==="LINK"&&p.rel==="modulepreload"&&l(p)}).observe(document,{childList:!0,subtree:!0});function n(s){const c={};return s.integrity&&(c.integrity=s.integrity),s.referrerPolicy&&(c.referrerPolicy=s.referrerPolicy),s.crossOrigin==="use-credentials"?c.credentials="include":s.crossOrigin==="anonymous"?c.credentials="omit":c.credentials="same-origin",c}function l(s){if(s.ep)return;s.ep=!0;const c=n(s);fetch(s.href,c)}})();const N="https://cqxpjesovcavarrlljpn.supabase.co",L="eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6ImNxeHBqZXNvdmNhdmFycmxsanBuIiwicm9sZSI6ImFub24iLCJpYXQiOjE3NzI0NjI4MjYsImV4cCI6MjA4ODAzODgyNn0._paXbtV1uecR0AA08KUkKLfRkmkQw2k4W3hT5L2y2ho";async function _(){const e=await fetch(`${N}/rest/v1/manual_events?select=*&order=date.asc`,{headers:{apikey:L,Authorization:"Bearer "+L}});return e.ok?(await e.json()).map(n=>({id:n.id,title:n.title,type:n.type,date:n.date,time:n.time,locationId:n.location_id,locationName:n.location_name,locationCity:n.location_city,description:n.description,ticketUrl:n.ticket_url,spotifyUrl:n.spotify_url,source:"manual"})):[]}async function q(e,t,n){return(await fetch(`${N}/rest/v1/manual_events`,{method:"POST",headers:{apikey:L,Authorization:"Bearer "+L,"Content-Type":"application/json",Prefer:"return=minimal"},body:JSON.stringify({id:e.id,title:e.title,type:e.type,date:e.date,time:e.time,location_id:e.locationId,location_name:t,location_city:n,description:e.description,ticket_url:e.ticketUrl,spotify_url:e.spotifyUrl})})).ok}function I(e){const t=localStorage.getItem(e);return t?JSON.parse(t):null}function k(e,t){localStorage.setItem(e,JSON.stringify(t))}function U(){const e=I("locations");if(e)return e;const t=[{id:1,name:"Lido",city:"Berlin",website:"lido-berlin.de",capacity:400},{id:2,name:"SO36",city:"Berlin",website:"so36.com",capacity:600},{id:3,name:"Festsaal Kreuzberg",city:"Berlin",website:"festsaal-kreuzberg.de",capacity:1200},{id:4,name:"Privatclub",city:"Berlin",website:"privatclub-berlin.de",capacity:250},{id:5,name:"Astra Kulturhaus",city:"Berlin",website:"astra-berlin.de",capacity:1800},{id:6,name:"Frannz Club",city:"Berlin",website:"frannz.eu",capacity:350},{id:7,name:"Monarch",city:"Berlin",website:"kottimonarch.de",capacity:150},{id:8,name:"Musik & Frieden",city:"Berlin",website:"musikundfrieden.de",capacity:700},{id:9,name:"Wild at Heart",city:"Berlin",website:"wildatheartberlin.de",capacity:280},{id:10,name:"Columbia Theater",city:"Berlin",website:"columbia-theater.de",capacity:800},{id:11,name:"Schokoladen",city:"Berlin",website:"schokoladen-mitte.de",capacity:150},{id:12,name:"Madame Claude",city:"Berlin",website:"madameclaude.de",capacity:100},{id:13,name:"Conne Island",city:"Leipzig",website:"conne-island.de",capacity:600},{id:14,name:"Werk 2",city:"Leipzig",website:"werk-2.de",capacity:500},{id:15,name:"Täubchenthal",city:"Leipzig",website:"taeubchenthal.com",capacity:1200},{id:16,name:"Felsenkeller",city:"Leipzig",website:"felsenkeller-leipzig.com",capacity:2e3},{id:17,name:"UT Connewitz",city:"Leipzig",website:"utconnewitz.de",capacity:350},{id:18,name:"Moritzbastei",city:"Leipzig",website:"moritzbastei.de",capacity:500},{id:19,name:"Horns Erben",city:"Leipzig",website:"horns-erben.de",capacity:150},{id:20,name:"Ilses Erika",city:"Leipzig",website:"",capacity:200},{id:21,name:"Urban Spree",city:"Berlin",website:"urbanspree.com",capacity:250},{id:22,name:"Gretchen",city:"Berlin",website:"gretchen-club.de",capacity:500},{id:23,name:"Supamolly",city:"Berlin",website:"supamolly.de",capacity:150},{id:24,name:"Kantine am Berghain",city:"Berlin",website:"berghain.berlin",capacity:200},{id:25,name:"Tempodrom",city:"Berlin",website:"tempodrom.de",capacity:4200}];return k("locations",t),t}async function M(){try{const e=await fetch("./events.json"),t=e.ok?await e.json():[],n=await _();return[...t,...n]}catch(e){return console.log("Fehler beim Laden:",e),[]}}let g=U(),x=[],T=null,r={cities:["Berlin","Leipzig"],type:"alle",locationId:"alle",dates:[]},m=I("bookmarked")||[],h=I("going")||[],S=I("seenEvents")||[],v="liste",B=0;async function P(e){T=e,x=await M(),u()}function j(e){return!S.includes(e.id)}function K(){S=x.map(e=>e.id),k("seenEvents",S)}function u(){const e=x.filter(t=>j(t)).length;T.innerHTML=`
    ...
    ${Y()}
  `,X(),e>0&&v==="liste"&&setTimeout(()=>K(),3e3)}function R(e){return`
    ...
    <div class="space-y-8">
      ${Object.entries(n).map(([l,s])=>`
        <div>
          ...
          <div class="space-y-3">${s.map(c=>A(c)).join("")}</div>
        </div>
      `).join("")}
    </div>
  `}function A(e){const t=g.find(d=>d.id===e.locationId),n=m.includes(e.id),l=h.includes(e.id),s=j(e),p=new Date(e.date+"T12:00:00").toLocaleDateString("de-DE",{weekday:"short",day:"numeric",month:"short"}),a=e.type==="konzert",i=a?"#818cf8":"#fb923c";return`
  ...
  `}function W(){const e=new Date,t=new Date(e.getFullYear(),e.getMonth()+B,1),n=t.getFullYear(),l=t.getMonth(),s=new Date(n,l+1,0).getDate(),c=(new Date(n,l,1).getDay()+6)%7,p=t.toLocaleDateString("de-DE",{month:"long",year:"numeric"}),a=C(),i=a.map(d=>d.date),o=a.filter(d=>r.dates.length===0||r.dates.includes(d.date));return`
    ${D()}
    <div class="glass rounded-2xl p-5 mb-5">
      ...
      <div class="grid grid-cols-7 gap-1">
        ...
        ${Array.from({length:s},(d,E)=>{const w=E+1,y=`${n}-${String(l+1).padStart(2,"0")}-${String(w).padStart(2,"0")}`,f=i.includes(y),$=r.dates.includes(y),z=w===e.getDate()&&l===e.getMonth()&&n===e.getFullYear(),O=new Date(y)<e&&!z;return`
            <button data-date="${y}" style="aspect-ratio:1; border-radius:12px; font-size:14px; font-weight:500; display:flex; align-items:center; justify-content:center; position:relative; transition:all 0.15s; ${O?"opacity:0.25;":""} color:${!f&&!$&&!z?"rgba(255,255,255,0.2)":"white"}; background:${$?"rgba(99,102,241,0.5)":f?"rgba(99,102,241,0.12)":"transparent"}; border:${$?"1px solid rgba(99,102,241,0.6)":f?"1px solid rgba(99,102,241,0.25)":z?"1px solid rgba(255,255,255,0.2)":"1px solid transparent"}; padding:4px; cursor:pointer;">
              ${w}
              ${f&&!$?'<span style="position:absolute;bottom:3px;left:50%;transform:translateX(-50%);width:3px;height:3px;border-radius:50%;background:#818cf8;"></span>':""}
            </button>
          `}).join("")}
      </div>
      ${r.dates.length>0?`
    ...
    <div class="space-y-3">
      ${r.dates.length>0&&o.length===0?'<p class="text-center text-slate-600 py-6 text-sm">Keine Events an den gewählten Tagen.</p>':o.map(d=>A(d)).join("")}
    </div>
  `}function J(){const e=x.filter(l=>m.includes(l.id)),t=x.filter(l=>h.includes(l.id)),n=(l,s,c)=>`
    <div>
      <div class="flex items-center gap-3 mb-4">
        <div class="h-px flex-1" style="background:rgba(255,255,255,0.06);"></div>
        <span class="syne text-xs tracking-widest uppercase" style="color:rgba(255,255,255,0.25); font-weight:700;">${l}</span>
      ...
      ${s.length===0?`<p class="text-slate-700 text-sm py-4 text-center">${c}</p>`:`<div class="space-y-3">${s.map(p=>A(p)).join("")}</div>`}
    </div>
  `;return`
